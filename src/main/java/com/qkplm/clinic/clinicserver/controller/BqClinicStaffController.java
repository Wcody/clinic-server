/*
* 版权声明 Copyright (c) 2026。
* 版权所有者： [九维无纸化病案管理系统]
* 首创日期： 2026年4月14日
*/
package com.qkplm.clinic.clinicserver.controller;

import com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import org.springframework.web.bind.annotation.*;
import org.springframework.web.bind.annotation.RestController;
import com.qkplm.clinic.clinicserver.service.IBqClinicStaffService;
import com.qkplm.clinic.clinicserver.entity.BqClinicStaffEntity;
import com.qkplm.clinic.libcommon.annotation.BQAuthMark;
import com.qkplm.clinic.libcommon.annotation.BQLogMark;
import com.qkplm.clinic.libcommon.api.BQApiException;
import com.qkplm.clinic.libcommon.api.BQSearchParams;
import com.qkplm.clinic.libcommon.api.BQSearchParamsGet;

import java.io.Serializable;
import java.util.Collection;

/**
 * @author Wcke
 * @description <p>人员管理表 前端控制器</p>
 * @datetime 2026-4-14 12:56
 */
@RestController
@RequestMapping("/ams/api/v1/clinic/staff")
public class BqClinicStaffController {
    private final static String MODULE_NAME = "人员管理表";
    private final static String TAG_NAME = "clinicStaff";
    private final IBqClinicStaffService clinicStaffService;

    public BqClinicStaffController(IBqClinicStaffService clinicStaffService) {
        this.clinicStaffService = clinicStaffService;
    }

    /**
    * 根据ID获取单条记录
    */
    @BQAuthMark(tag = TAG_NAME, buttons = {})
    @BQLogMark(module = MODULE_NAME, operation = "查询")
    @RequestMapping(value = "/get/{id}", method = RequestMethod.GET)
    public BqClinicStaffEntity get(@PathVariable String id) {
        BqClinicStaffEntity clinicStaff = clinicStaffService.getById(id);
        if (clinicStaff == null) {
            throw new BQApiException("获取失败，对象可能不存在");
        }
        return clinicStaff;
    }

    /**
    * 新增单条记录
    */
    @BQAuthMark(tag = TAG_NAME, buttons = {})
    @BQLogMark(module = MODULE_NAME, operation = "新增")
    @RequestMapping(value = "/save", method = RequestMethod.POST)
    public BqClinicStaffEntity save(@RequestBody BqClinicStaffEntity clinicStaff) {
        clinicStaff.setId(null);
        if (!clinicStaffService.save(clinicStaff)) {
            throw new BQApiException("新增失败");
        }
        return clinicStaff;
    }

    /**
    * 批量新增记录，默认单次执行1000条
    */
    @BQAuthMark(tag = TAG_NAME, buttons = {})
    @BQLogMark(module = MODULE_NAME, operation = "批量新增")
    @RequestMapping(value = "/saveBatch", method = RequestMethod.POST)
    public Boolean saveBatch(@RequestBody Collection<BqClinicStaffEntity> clinicStaffs) {
        if (!clinicStaffService.saveBatch(clinicStaffs)) {
            throw new BQApiException("批量新增失败");
        }
        return true;
    }

    /**
    * 根据ID更新单条记录
    */
    @BQAuthMark(tag = TAG_NAME, buttons = {})
    @BQLogMark(module = MODULE_NAME, operation = "更新")
    @RequestMapping(value = "/update", method = RequestMethod.POST)
    public BqClinicStaffEntity update(@RequestBody BqClinicStaffEntity clinicStaff) {
        if (!clinicStaffService.updateById(clinicStaff)) {
            throw new BQApiException("更新失败，对象可能已失效");
        }
        return clinicStaff;
    }

    /**
    * 批量更新记录，默认单次执行1000条
    */
    @BQAuthMark(tag = TAG_NAME, buttons = {})
    @BQLogMark(module = MODULE_NAME, operation = "批量更新")
    @RequestMapping(value = "/updateBatch", method = RequestMethod.POST)
    public Boolean updateBatch(@RequestBody Collection<BqClinicStaffEntity> clinicStaffs) {
        if (!clinicStaffService.updateBatchById(clinicStaffs)) {
            throw new BQApiException("批量更新失败");
        }
        return true;
    }


    /**
    * 根据ID删除单条记录，物理删除
    */
    @BQAuthMark(tag = TAG_NAME, buttons = {})
    @BQLogMark(module = MODULE_NAME, operation = "删除")
    @RequestMapping(value = "/delete/{id}", method = RequestMethod.GET)
    public Boolean delete(@PathVariable String id) {
        if (!clinicStaffService.removePhysicalById(id)) {
            throw new BQApiException("删除失败，对象可能不存在");
        }
        return true;
    }

    /**
    * 批量删除记录,物理删除，默认单次执行1000条
    */
    @BQAuthMark(tag = TAG_NAME, buttons = {})
    @BQLogMark(module = MODULE_NAME, operation = "批量删除")
    @RequestMapping(value = "/deleteBatch", method = RequestMethod.POST)
    public Boolean deleteBatch(@RequestBody Collection<Serializable> ids) {
        if (!clinicStaffService.removePhysicalBatchByIds(ids)) {
            throw new BQApiException("批量删除失败");
        }
        return true;
    }

    /**
    * 根据ID删除单条记录，逻辑删除
    */
    @BQAuthMark(tag = TAG_NAME, buttons = {})
    @BQLogMark(module = MODULE_NAME, operation = "删除")
    @RequestMapping(value = "/deleteLogic/{id}", method = RequestMethod.GET)
    public Boolean deleteLogic(@PathVariable String id) {
        if (!clinicStaffService.removeById(id)) {
            throw new BQApiException("删除失败，对象可能不存在");
        }
        return true;
    }

    /**
    * 批量删除记录，逻辑删除，默认单次执行1000条
    */
    @BQAuthMark(tag = TAG_NAME, buttons = {})
    @BQLogMark(module = MODULE_NAME, operation = "批量删除")
    @RequestMapping(value = "/deleteLogicBatch", method = RequestMethod.POST)
    public Boolean deleteLogicBatch(@RequestBody Collection<Serializable> ids) {
        if (!clinicStaffService.removeBatchByIds(ids)) {
            throw new BQApiException("批量删除失败");
        }
        return true;
    }

    /**
    * 列表查询，orders可以排序，filters可以过滤
    */
    @BQAuthMark(tag = TAG_NAME, buttons = {})
    @BQLogMark(module = MODULE_NAME, operation = "列表查询")
    @RequestMapping(value = "/list", method = RequestMethod.GET)
    public Iterable<BqClinicStaffEntity> list(BQSearchParamsGet<BqClinicStaffEntity> paramsGet) {
        BQSearchParams<BqClinicStaffEntity> params = paramsGet.toSearchParams();
        return clinicStaffService.list(params.toPage(), params.toWrapper());
    }

    /**
    * 获取医生列表（仅启用的）
    */
    @BQAuthMark(tag = TAG_NAME, buttons = {})
    @BQLogMark(module = MODULE_NAME, operation = "医生列表查询")
    @RequestMapping(value = "/listDoctors", method = RequestMethod.GET)
    public Iterable<BqClinicStaffEntity> listDoctors() {
        LambdaQueryWrapper<BqClinicStaffEntity> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(BqClinicStaffEntity::getStatus, "启用")
               .eq(BqClinicStaffEntity::getDeleted, false)
               .orderByAsc(BqClinicStaffEntity::getName);
        return clinicStaffService.list(wrapper);
    }
}
